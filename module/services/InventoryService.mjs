export async function onDropItem(event) {
	...
	if (item.type === 'skill') {
		return ui.notifications.error(
			'Skills cannot be dropped in the inventory.'
		);
	}
	...
	new Dialog(
		{
				...
				yes: {
					...
					callback: () => {
						if (data.coins >= cost) {
							...
							this.actor
								.update(updateData)
								.then(() => this.render()); // Re-render the sheet
						} else {
							ui.notifications.error(
								'Not enough coins to expand inventory!'
							);
						}
					},
				},
			...
			default: 'no',
			close: () =>
				console.log('Expand inventory closed without choosing.'),
		},
		dialogOptions
	).render(true);
}
